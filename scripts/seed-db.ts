...
const generateRandomTransactions = (
  ...
  const categories = Array.from(categoryMap.keys()).filter(
    (name) => name !== "Gaji" && name !== "Bonus",
  );
...
async function main() {
  ...
  const allCats = [
    ...
    { name: "Tabungan", color: "#10b981", priority: "Keinginan" },
    { name: "Gaji", color: "#10b981", type: "income" },
    { name: "Bonus", color: "#8b5cf6", type: "income" },
  ];
